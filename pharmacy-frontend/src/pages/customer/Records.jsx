  ...
  return (
    <AppLayout>
      <div className="mb-8 animate-in slide-in-from-bottom-4 duration-500">
        <h1 className="text-3xl font-bold text-slate-800">Health Records</h1>
        <p className="text-slate-500 mt-1 text-lg">View your order history and prescriptions</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-6 duration-500 delay-100 fill-mode-both">
        {/* Order History */}
            ...
            <div className="divide-y divide-slate-100">
              {mockOrders.map(order => (
                <div key={order.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/80 transition-colors">
                  <div>
                    <h4 className="font-bold text-slate-800 text-lg mb-1">{order.id}</h4>
                    ...
                    <p className="text-sm text-slate-600">{order.items} Items <span className="mx-2 text-slate-300">•</span> <span className="font-bold text-slate-800">${order.total.toFixed(2)}</span></p>
                  </div>
